...
class ConjFeat:
    def rgb_to_bgr(self,im):
        ...
        om[:,:,0] = im[:,:,2]
        return om
    def __init__(self,image,hhr=50,input_channels='rgb'): #revisar
        """
    ...
    def calc_entropy(self,v=False):
        ...
        equ            = eq.equalize_image(self.image[:,:,1])
    ...
    def calc_HHR(self,v=False):
        ...
        hsv      = colors.rgb_to_hsv(self.image)
        #plt.imshow(hsv)
        self.HHR = 0
    ...
    def calc_bright(self,v=False):
        ...
        set v=True for information when it is calculated
        """
        r= np.mean(self.image[:,:,0]);g= np.mean(self.image[:,:,1]);b= np.mean(self.image[:,:,2])
        #Brightness = np.sqrt(0.241*self.image[:,:,0] + 0.691*self.image[:,:,1] + 0.68*self.image[:,:,2])
        Brightness      = np.sqrt(0.241*(r**2) + 0.691*(g**2) + 0.68*(b**2))
